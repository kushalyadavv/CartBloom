...
### The shared entitlement core

A single TypeScript module answers: *given this cart and this config, which gifts are entitled?* It is compiled into **both** the Wasm discount function and the widget bundle.
...
1. **Javy instruction ceiling.** A TypeScript entitlement core compiled to Wasm consumes substantially more instructions than Rust. **Benchmark in week one, before the shared-core design is built upon.** Fallback if exceeded: port only the entitlement core to Rust and generate the widget's copy from the same specification — worse, but survivable.
2. **JSON metafield size limit.** A merchant with many offers and large gift pools could produce a large config. Measure against a realistic maximum before shipping.
...
| Function language | TypeScript via Javy | Enables the shared entitlement core; instruction limit is the tracked risk |
