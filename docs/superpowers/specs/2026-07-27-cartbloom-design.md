...
3. **Market/country availability on the discount function input.** Still unconfirmed as of 2026-07-27 -- published pages are suggestive but not definitive, and the input schema pages 404. **Resolved definitively in Phase 2 Task 20**, which records the `schema.graphql` the CLI actually generates; that file is authoritative. If `localization` is absent, market targeting drops from Pro.
