...
Requires: cart lines (id, quantity, `cost.amountPerQuantity`, merchandise variant + product with `inAnyCollection`, line attributes), `cart.cost.subtotalAmount`, `buyerIdentity.customer.hasAnyTag(tags:)`, the discount node metafield, and `localization.country.isoCode` (confirmed present; do not use the deprecated `localization.market`).
...
3. ~~**Market/country availability on the discount function input.**~~ **RESOLVED 2026-07-27** by inspecting the CLI-generated `schema.graphql`. `Localization` exposes `country: Country!` (with `isoCode`) and `language`, so market targeting is deliverable -- but `localization.market` is **deprecated and slated for removal**. Target on `localization.country.isoCode` (ISO 3166-1 alpha-2); §5's `audience.markets[]` holds country codes. See `docs/superpowers/plans/rust-toolchain-notes.md`.
