...
**Architecture:** The entitlement core is a dependency-free TypeScript module exposing pure functions. It accepts a normalised cart (each line pre-resolved for scope by its host) plus an offer config, and returns entitlements. It imports nothing — no Shopify SDK, no DOM, no Node built-ins — so it compiles unchanged into both a Javy-built Wasm function and a browser bundle. Task 1 proves the instruction budget before anything is built on this assumption.
...
- [ ] Instruction budget measured, recorded, and inside the gate from Task 1 Step 7
...
**Phase 2 is unblocked** once both spikes pass. Phase 2 wraps this core in the Wasm discount function, builds the production Workers + D1 setup informed by Task 13, resolves collection scope into the `inScope` flags, and verifies a real discount applying at a real checkout.
